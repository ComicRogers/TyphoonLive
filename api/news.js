/* ============================================================
 * api/news.js — 台风新闻聚合 API (Vercel Serverless Function)
 *
...
// 天行数据 key,部署后在 Vercel 环境变量中设置 TIANAPI_KEY
const DEFAULT_TIANAPI_KEY = process.env.TIANAPI_KEY || '';
...
module.exports = async function handler(req, res) {
  ...
  if (!name) {
    res.status(400).json({ error: '缺少台风名称参数 ?name=' });
    return;
  }

  const tianKey = key || DEFAULT_TIANAPI_KEY;
  ...
  res.status(200).json({ items: relevant });
};
